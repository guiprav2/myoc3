  ...
  render = () => d.html`
    <div class="relative bg-neutral-200">
      <div class="mx-auto max-w-5xl shadow-xl bg-neutral-100 min-h-screen flow-root">
        ...
        ${d.if(() => this.state.profile.bio, d.html`
          <div>
            ...
            <div class="p-4 grid grid-cols-2 gap-2 text-sm items-center">
              ${d.map(() => Object.keys(this.state.profile.bio), x => d.html`
                <div>${x}:</div>
                <input class="outline-none px-3 py-2" ${{
                  value: d.binding({ get: () => this.state.profile.bio[x], set: y => this.state.profile.bio[x] = y }),
                }}>
              `)}
            </div>
          </div>
        `)}
        ${d.if(() => this.state.profile.physical, d.html`
          <div>
            <div class="bg-[#FFA1C3] text-white px-4 py-1">Physical Description</div>
            <div class="p-4 grid grid-cols-2 gap-2 text-sm items-center">
              ${d.map(() => Object.keys(this.state.profile.physical), x => d.html`
                <div>${x}:</div>
                <input class="outline-none px-3 py-2" ${{
                  value: d.binding({ get: () => this.state.profile.physical[x], set: y => this.state.profile.physical[x] = y }),
                }}>
              `)}
            </div>
          </div>
        `)}
        ${d.if(() => this.state.profile.gallery, d.html`
          <div>
        ...
        `)}
        ${d.if(() => !this.state.profile.bio || !this.state.profile.physical || !this.state.profile.gallery, d.html`
              ...
              <div class="bg-white border border-[#00000010] shadow-xl rounded text-sm overflow-hidden text-[#A7A7A7]">
                ${d.if(() => !this.state.profile.bio, d.html`
                  <a class="px-3 py-2 border-b border-[#00000010] hover:bg-[#FFA1C3] hover:text-white block" ${{
                    onClick: () => this.post('profile.createBio'),
                  }}>
                ...
                ${d.if(() => !this.state.profile.physical, d.html`
                  <a class="px-3 py-2 border-b border-[#00000010] hover:bg-[#FFA1C3] hover:text-white block" ${{
                    onClick: () => this.post('profile.createPhysical'),
                  }}>
                ...
                ${d.if(() => !this.state.profile.gallery, d.html`
                  <a class="px-3 py-2 border-b border-[#00000010] hover:bg-[#FFA1C3] hover:text-white block" ${{
                    onClick: () => this.post('profile.createGallery'),
                  }}>
                    Gallery
                  </a>
                `)}
